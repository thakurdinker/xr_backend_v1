...
router.route("/sitemap").get(async (req, res) => {
  ...
  // ── Default: serve from MongoDB (shared across all instances) ──
  try {
    const sitemap = await Sitemap.findOne({}).lean();
    if (sitemap?.content) {
      res.set("Content-Type", "application/xml");
      res.set("Cache-Control", "public, max-age=3600");
      res.set("X-Sitemap-Source", "mongodb");
      res.set("X-Sitemap-Url-Count", String(sitemap.urlCount || 0));
      return res.send(sitemap.content);
    }
  } catch (err) {
    console.error("[Sitemap] MongoDB read failed:", err.message);
  }

  // Fallback — try local file (in case MongoDB save failed)
  try {
    if (fs.existsSync(SITEMAP_FILE)) {
      const data = fs.readFileSync(SITEMAP_FILE, "utf8");
      res.set("Content-Type", "application/xml");
      res.set("Cache-Control", "public, max-age=3600");
      res.set("X-Sitemap-Source", "local-fallback");
      return res.send(data);
    }
  } catch (err) {
    console.error("[Sitemap] Local file read failed:", err.message);
  }

  // Last resort — generate on the fly
  console.log("[Sitemap] No sitemap found, generating...");
  const result = await generateSitemap();
  if (result.success) {
    const sitemap = await Sitemap.findOne({}).lean();
    if (sitemap?.content) {
      ...
      res.set("X-Sitemap-Source", "mongodb-generated");
      return res.send(sitemap.content);
    }
  }
